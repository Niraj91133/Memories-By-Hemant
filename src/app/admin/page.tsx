...
export default function AdminPage() {
  ...
  const [diagnostics, setDiagnostics] = useState<{
    supabase: { configured: boolean; connected: boolean; url: string | null; error: string | null };
    cloudinary: { configured: boolean; connected: boolean; cloudName: string | null; error: string | null };
  } | null>(null);
  ...
  const sectionContent = media.filter(item => {
    ...
    if (activeSection === "Hero" && item.title === "LOGO") return false;
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-[#FDFBF7] flex flex-col lg:flex-row relative selection:bg-[#830F1D] selection:text-white">
      ...
      <aside className="w-full lg:w-80 h-auto lg:h-screen bg-[#111111] border-b lg:border-b-0 lg:border-r border-white/5 flex flex-col z-10 shrink-0">
        ...
        <div className="px-8 py-4 border-b border-white/5 bg-white/[0.01] flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <span className="text-[9px] font-bold tracking-widest text-white/40 uppercase">DATABASE:</span>
            {!diagnostics ? (
              <span className="text-[8px] font-black tracking-widest text-white/20 animate-pulse uppercase">CHECKING...</span>
            ) : diagnostics.supabase.connected ? (
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[8px] font-black tracking-widest bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 uppercase">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                LIVE
              </span>
            ) : diagnostics.supabase.configured ? (
              <span 
                title={diagnostics.supabase.error || "Supabase configuration error"} 
                className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[8px] font-black tracking-widest bg-red-500/10 text-red-400 border border-red-500/20 uppercase cursor-help"
              >
                🔴 ERROR
              </span>
            ) : (
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[8px] font-black tracking-widest bg-amber-500/10 text-amber-400 border border-amber-500/20 uppercase">
                🟡 LOCAL MOCK
              </span>
            )}
          </div>
          ...
          <div className="flex items-center justify-between">
            <span className="text-[9px] font-bold tracking-widest text-white/40 uppercase">STORAGE:</span>
            {!diagnostics ? (
              <span className="text-[8px] font-black tracking-widest text-white/20 animate-pulse uppercase">CHECKING...</span>
            ) : diagnostics.cloudinary.connected ? (
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[8px] font-black tracking-widest bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 uppercase">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                LIVE
              </span>
            ) : diagnostics.cloudinary.configured ? (
              <span 
                title={diagnostics.cloudinary.error || "Cloudinary configuration error"} 
                className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[8px] font-black tracking-widest bg-red-500/10 text-red-400 border border-red-500/20 uppercase cursor-help"
              >
                🔴 ERROR
              </span>
            ...
            <div className="flex items-center gap-4">
                {hasUnsavedChanges && (
                  <motion.div 
                    ...
                    className="flex items-center gap-2 bg-[#830F1D]/15 border border-[#830F1D]/30 px-4 py-2 rounded-full mr-2"
                  >
                    <span className="w-1.5 h-1.5 rounded-full bg-[#830F1D] animate-ping" />
                    <span className="text-[8px] font-black tracking-widest text-[#830F1D] uppercase">UNPUBLISHED CHANGES</span>
                  </motion.div>
                )}
         ...
         <div className="flex flex-col gap-12">
                ...
                {faqs.map((faq, i) => (
                  <div key={i} className="flex flex-col gap-4 bg-[#111111] p-6 border border-white/5 rounded-2xl relative group">
                    <div className="flex justify-between items-start">
                      ...
                      <button onClick={() => { setFaqs(prev => prev.filter((_, idx) => idx !== i)); setHasUnsavedChanges(true); }} className="text-white/20 hover:text-red-500 transition-colors uppercase text-[9px] font-black shrink-0">REMOVE</button>
                    </div>
          ...
          {activeSection !== "FAQ" && activeSection !== "About" && activeSection !== "Settings" && (
            <>
              ...
              {activeSection === "Hero" && (
                <div className="bg-[#111111] border border-white/5 rounded-3xl p-8 mb-8 flex flex-col sm:flex-row items-center gap-8 justify-between">
                  <div className="flex flex-col gap-2">
                    <span className="text-[10px] font-black tracking-[0.5em] text-[#830F1D] uppercase">Website Logo</span>
                    <p className="text-white/40 text-[10px] font-bold tracking-widest uppercase">
                      This logo will appear on the navigation bar and the main hero header.
                    </p>
                    <div className="flex gap-4 mt-4">
                      <button
                        onClick={() => logoInputRef.current?.click()}
                        disabled={isUploading}
                        className={`bg-white text-black px-6 py-3 rounded-full font-black text-[10px] tracking-[0.2em] uppercase transition-all transform active:scale-95 ${
                          isUploading ? "opacity-50 cursor-not-allowed" : "hover:bg-[#830F1D] hover:text-white"
                        }`}
                      >
                        {isUploading ? "UPLOADING..." : "UPLOAD NEW LOGO"}
                      </button>
                      {media.some(m => m.section === "Hero" && m.title === "LOGO") && (
                        <button
                          onClick={async () => {
                            const logoItem = media.find(m => m.section === "Hero" && m.title === "LOGO");
                            if (logoItem && logoItem.id && !logoItem.id.includes('default')) {
                              await deleteMediaItem(logoItem.id);
                            }
                            setMedia(prev => prev.filter(m => !(m.section === "Hero" && m.title === "LOGO")));
                            setHasUnsavedChanges(true);
                            showNotification("Logo removed. Default typography will be used.", "info");
                          }}
                          className="border border-white/10 text-white/60 hover:text-red-500 hover:border-red-500/30 px-6 py-3 rounded-full font-black text-[10px] tracking-[0.2em] uppercase transition-all transform active:scale-95"
                        >
                          REMOVE LOGO
                        </button>
                      )}
                  ...
                  {/* Logo Preview */}
                  <div className="w-full sm:w-80 h-32 bg-[#0A0A0A] border border-white/10 rounded-2xl flex items-center justify-center p-6 relative overflow-hidden group">
                    {media.find(m => m.section === "Hero" && m.title === "LOGO")?.url ? (
                      <div className="relative w-full h-full">
                        <Image
                          src={media.find(m => m.section === "Hero" && m.title === "LOGO")!.url}
                      ...
                      <div className="flex flex-col items-center gap-1 opacity-20">
                        ...
                        <span className="text-[8px] font-black tracking-widest uppercase">No logo uploaded</span>
                      </div>
                    )}
                  </div>
                </div>
