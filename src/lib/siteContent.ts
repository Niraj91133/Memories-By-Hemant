...
export async function fetchSiteContent(): Promise<SiteContent> {
  ...
  try {
    ...
    let filteredMedia = media && media.length > 0 ? media : defaults.media;
    if (filteredMedia.length > 0) {
      const logos = filteredMedia.filter((m: any) => m.section === "Hero" && m.title === "LOGO");
      if (logos.length > 1) {
        const targetLogo = logos.find((l: any) => l.id === "website-logo") || logos[0];
        filteredMedia = filteredMedia.filter((m: any) => !(m.section === "Hero" && m.title === "LOGO") || m.id === targetLogo.id);
        
        // Clean up duplicate logos from the database in the background
        const idsToDelete = logos.filter((l: any) => l.id !== targetLogo.id).map((l: any) => l.id);
        for (const deleteId of idsToDelete) {
          supabase.from('media_items').delete().eq('id', deleteId).then().catch((err: any) => {
            console.error(`Failed to delete duplicate logo ${deleteId}:`, err);
          });
        }
      }
    }
